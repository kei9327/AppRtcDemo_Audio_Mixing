 ...
 *
 * NOTI : Kotlin 으로 변환하지 말 것.
 *        Java Object 객체를 참조해야 함.
 */
public class LooperExecutor extends Thread implements Executor {
    ...
    public synchronized void queue(final Runnable runnable) {
        if (!running) {
            Log.w(TAG, "Running looper executor without calling requestStart()");
            return;
        }
    ...
    public synchronized void queue(final Runnable runnable, long delayMillis) {
        if (!running) {
            Log.w(TAG, "Running looper executor without calling requestStart()");
            return;
        }
