...
public class WebRtcAudioRecord {
  ...
  public static class AudioMixSamples {
    ...
    private AudioMixSamples(AudioRecord audioRecord, ByteBuffer data) {
      this.audioFormat = audioRecord.getAudioFormat();
      this.channelCount = audioRecord.getChannelCount();
      this.sampleRate = audioRecord.getSampleRate();
      this.data = data;
  ...
  public static interface WebRtcAudioRecordSamplesMixAvailableCallback {
    byte[] onWebRtcAudioRecordSamplesMixAvailable(AudioMixSamples samples);
  }

  private static @Nullable WebRtcAudioRecordSamplesMixAvailableCallback audioSamplesMixAvailableCallback;

  public static void setOnAudioSamplesMixAvailable(WebRtcAudioRecordSamplesMixAvailableCallback callback) {
    audioSamplesMixAvailableCallback = callback;
  }
    ...
    public void run() {
      ...
      while (keepAlive) {
        ...
        if (bytesRead == byteBuffer.capacity()) {
          ...
          // BGM, 효과음 처리를 위한 콜백
          if (audioSamplesMixAvailableCallback != null) {
            byteBuffer.clear();
            byteBuffer.put(audioSamplesMixAvailableCallback.onWebRtcAudioRecordSamplesMixAvailable(new AudioMixSamples(audioRecord, byteBuffer)));
          }
